...
-- Cách 1:
-- +GP1: like '%keyword%'
-- +GP2: like'keyword%' + index (nhưng id và phone đã có index) 

-- Triển khai 
-- 1.Delimiter //
-- 2.create procedure procedure_name(in,on)
   -- B2.1:Tạo store proceduce
   -- B2.1:Truyền vào các tham số (in gồm keyword và out gồm tổng nợ và thông báo)
-- 3.begin
   -- Kiểm tra tính hợp lệ của các tham số đầu vào
   -- Viết câu lệnh truy vấn
   -- Trả về kết quả nếu có dữ liệu và không có dữ liệu 
-- 4.end //
-- delimiter ;
...
begin 
   ...
   declare massage_error varchar(100);
   -- Kiểm tra dữ liệu người dùng nhập vào 
   if keyword is null then set massage = 'Không được phép truy xuất';
   end if;
   select * 
   from Patient_Invoices pi
   join patients p
   on p.patient_id = pi.patient_id
   where pi.patient_id like 'keyword%' or phone like 'keyword%';
end //
delimiter ;
set @total_due_value = 10000;
set @massage_value = '';
call GetPatientDebt (null,@total_due_value,@massage_value);
